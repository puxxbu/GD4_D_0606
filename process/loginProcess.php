...
if (isset($_POST['login'])) {
    ...
    if (mysqli_num_rows($query) == 0) {
        ...
        if (password_verify($password, $user['password'])) {
            ...
            session_start();
            //isLogin ini temp variable yang gunanya buat ngecek nanti apakah sd login ato belum
            $_SESSION['isLogin'] = true;
            $_SESSION['user'] = $user;
            echo '<script> alert("Login Success"); window.location = "../page/dashboardPage.php"
                </script>';
        } else {
            echo
            '<script>
